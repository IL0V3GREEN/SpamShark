...
async def info_handling(call: CallbackQuery):
    ...
    if thing == "rating":
        await call.message.edit_text(
            ...
            f"- 1.25₽ за 1 сообщение при спам-рассылке\n\n"
            f"🟧 Легенда (от 1000 🏆):\n"
            f"- 20% от пополнений рефералов\n"
            f"- 1₽ за 1 сообщение при спам-рассылке\n"
            f"- Ты. Легенда.\n\n"
...
async def rating_handlers(call: CallbackQuery):
    ...
    if action == "increase":
        await call.message.edit_text(
            ...
            f"Ссылка: <code>https://t.me/spamsharkbot?start={call.from_user.id}</code>\n",

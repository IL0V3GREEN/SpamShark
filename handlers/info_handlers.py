...
async def info_handling(call: CallbackQuery):
    ...
    if thing == "rating":
        await call.message.edit_text(
            ...
            f"└ +<code>5</code>% от пополнений рефералов\n\n"
            f"🟪 Мейкер (от <code>500</code> 🏆):\n"
            f"├ +<code>10</code>% от пополнений рефералов\n"
            f"└ -<code>0.25</code>₽ за <code>1</code> сообщение при спам-рассылке\n\n"
            f"🟧 Легенда (от <code>1000</code> 🏆):\n"
            f"├ +<code>15</code>% от пополнений рефералов\n"

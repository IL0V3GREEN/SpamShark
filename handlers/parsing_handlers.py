...
async def start_parsing(message: Message, state: FSMContext):
    if db.check_parse_available(message.from_user.id):
        await message.answer(
            "🌚 Каких пользователей парсим?",
            reply_markup=where_parsing()
        )
    else:
        await message.answer("Парсить можно только 3 раза в сутки.")
...
async def getting_spam_locate(call: CallbackQuery, state: FSMContext):
    ...
    if db.check_parse_available(call.from_user.id):
        if locate == "chat":
        ...
        await call.message.answer("Парсить можно только 3 раза в сутки.")
